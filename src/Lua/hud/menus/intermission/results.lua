...
return function(v)
	...
	for y = backgroundStartY, screenHeight, background.height * FU do
		for x = backgroundStartX, screenWidth, background.width * FU do
			v.drawScaled(x, y, FU, background, V_SNAPTOLEFT|V_SNAPTOTOP, v.getColormap(nil, skincolors[winner.color].invcolor))
		end
	end
	...
	v.drawScaled(portraitX + 4 * FU, portraitY + 4 * FU, FU, portrait, V_SNAPTORIGHT|V_SNAPTOBOTTOM, v.getColormap(TC_BLINK, winner.color))
	v.drawScaled(portraitX, portraitY, FU, portrait, V_SNAPTORIGHT|V_SNAPTOBOTTOM, v.getColormap(nil, winner.color))
	v.draw(0, 0, nameBackground, V_SNAPTOTOP|V_SNAPTOLEFT)

	local textY = 8
	SSL.drawString(v, 8, textY, winner.name, "LTFNT%03d", V_SNAPTOTOP|V_SNAPTOLEFT, 0, 0, skincolors[winner.color].chatcolor, 0, 0)
	textY = $ + 24

	SSL.drawString(v, 8, textY, "Recoded your mom!", "TNYFN%03d", V_SNAPTOTOP|V_SNAPTOLEFT, 0, 0, skincolors[winner.color].chatcolor)
	textY = $ + 10
